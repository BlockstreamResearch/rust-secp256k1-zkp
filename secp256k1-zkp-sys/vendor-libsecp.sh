...
if [ -z "$1" ] | [ -z "$2" ]; then
  ...
  echo "\$2 parameter must be the rust-secp256k1-zkp-sys version code (M_m_p format)"
...
rm -rf "$DIR"
git clone https://github.com/jonasnick/secp256k1-zkp.git "$DIR"
cd "$DIR"
if [ -n "$REV" ]; then
    git checkout "$REV"
fi
...
find "$DIR" -not -path '*/\.*' -type f -print0 | xargs -0 sed -i "/^#include/! s/secp256k1_/rustsecp256k1zkp_v${VERSIONCODE}_/g"
...
sed -i -r "s/^links = \".*\"$/links = \"rustsecp256k1zkp_v${VERSIONCODE}\"/" Cargo.toml

# Update the extern references in the Rust FFI source files.
find "./src/" -name "*.rs" -type f -print0 | xargs -0 sed -i -r "s/rustsecp256k1zkp_v[0-9]+_[0-9]+_[0-9]+_(.*)([\"\(])/rustsecp256k1zkp_v${VERSIONCODE}_\1\2/g"
