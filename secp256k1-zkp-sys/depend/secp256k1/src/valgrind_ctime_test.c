...
int main(void) {
    ...
    ret = rustsecp256k1zkp_v0_4_0_context_randomize(ctx, key);
...
void run_tests(rustsecp256k1zkp_v0_4_0_context *ctx, unsigned char *key) {
...
#ifdef ENABLE_MODULE_RECOVERY
    rustsecp256k1zkp_v0_4_0_ecdsa_recoverable_signature recoverable_signature;
    int recid;
#endif
#ifdef ENABLE_MODULE_EXTRAKEYS
    ...
    /* Test keygen. */
    VALGRIND_MAKE_MEM_UNDEFINED(key, 32);
    ret = rustsecp256k1zkp_v0_4_0_ec_pubkey_create(ctx, &pubkey, key);
    VALGRIND_MAKE_MEM_DEFINED(&pubkey, sizeof(rustsecp256k1zkp_v0_4_0_pubkey));
    VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
    CHECK(ret);
    CHECK(rustsecp256k1zkp_v0_4_0_ec_pubkey_serialize(ctx, spubkey, &outputlen, &pubkey, SECP256K1_EC_COMPRESSED) == 1);

    /* Test signing. */
    VALGRIND_MAKE_MEM_UNDEFINED(key, 32);
    ret = rustsecp256k1zkp_v0_4_0_ecdsa_sign(ctx, &signature, msg, key, NULL, NULL);
    VALGRIND_MAKE_MEM_DEFINED(&signature, sizeof(rustsecp256k1zkp_v0_4_0_ecdsa_signature));
    VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
    CHECK(ret);
    CHECK(rustsecp256k1zkp_v0_4_0_ecdsa_signature_serialize_der(ctx, sig, &siglen, &signature));

#ifdef ENABLE_MODULE_ECDH
    /* Test ECDH. */
    VALGRIND_MAKE_MEM_UNDEFINED(key, 32);
    ret = rustsecp256k1zkp_v0_4_0_ecdh(ctx, msg, &pubkey, key, NULL, NULL);
    VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
    CHECK(ret == 1);
#endif
    ...
    VALGRIND_MAKE_MEM_UNDEFINED(key, 32);
    ret = rustsecp256k1zkp_v0_4_0_ecdsa_sign_recoverable(ctx, &recoverable_signature, msg, key, NULL, NULL);
    VALGRIND_MAKE_MEM_DEFINED(&recoverable_signature, sizeof(recoverable_signature));
    VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
    CHECK(ret);
    CHECK(rustsecp256k1zkp_v0_4_0_ecdsa_recoverable_signature_serialize_compact(ctx, sig, &recid, &recoverable_signature));
    CHECK(recid >= 0 && recid <= 3);
#endif

    VALGRIND_MAKE_MEM_UNDEFINED(key, 32);
    ret = rustsecp256k1zkp_v0_4_0_ec_seckey_verify(ctx, key);
    VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
    CHECK(ret == 1);

    VALGRIND_MAKE_MEM_UNDEFINED(key, 32);
    ret = rustsecp256k1zkp_v0_4_0_ec_seckey_negate(ctx, key);
    VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
    CHECK(ret == 1);

    VALGRIND_MAKE_MEM_UNDEFINED(key, 32);
    VALGRIND_MAKE_MEM_UNDEFINED(msg, 32);
    ret = rustsecp256k1zkp_v0_4_0_ec_seckey_tweak_add(ctx, key, msg);
    VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
    CHECK(ret == 1);

    VALGRIND_MAKE_MEM_UNDEFINED(key, 32);
    VALGRIND_MAKE_MEM_UNDEFINED(msg, 32);
    ret = rustsecp256k1zkp_v0_4_0_ec_seckey_tweak_mul(ctx, key, msg);
    VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
    CHECK(ret == 1);
    ...
    ret = rustsecp256k1zkp_v0_4_0_keypair_xonly_tweak_add(ctx, &keypair, msg);
    VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
    CHECK(ret == 1);

    VALGRIND_MAKE_MEM_UNDEFINED(key, 32);
    VALGRIND_MAKE_MEM_UNDEFINED(&keypair, sizeof(keypair));
    ret = rustsecp256k1zkp_v0_4_0_keypair_sec(ctx, key, &keypair);
    VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
    CHECK(ret == 1);
#endif
...
#ifdef ENABLE_MODULE_ECDSA_S2C
    {
        ...
        ret = rustsecp256k1zkp_v0_4_0_ecdsa_s2c_sign(ctx, &signature, &s2c_opening, msg, key, s2c_data);
        VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
        CHECK(ret == 1);

        VALGRIND_MAKE_MEM_UNDEFINED(s2c_data, 32);
        ret = rustsecp256k1zkp_v0_4_0_ecdsa_anti_exfil_host_commit(ctx, s2c_data_comm, s2c_data);
        VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
        CHECK(ret == 1);

        VALGRIND_MAKE_MEM_UNDEFINED(key, 32);
        VALGRIND_MAKE_MEM_UNDEFINED(s2c_data, 32);
        ret = rustsecp256k1zkp_v0_4_0_ecdsa_anti_exfil_signer_commit(ctx, &s2c_opening, msg, key, s2c_data);
        VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
        CHECK(ret == 1);
    }
#endif
...
#ifdef ENABLE_MODULE_ECDSA_ADAPTOR
    {
        ...
        ret = rustsecp256k1zkp_v0_4_0_ec_pubkey_create(ctx, &enckey, deckey);
        CHECK(ret == 1);

        VALGRIND_MAKE_MEM_UNDEFINED(key, 32);
        ret = rustsecp256k1zkp_v0_4_0_ecdsa_adaptor_encrypt(ctx, adaptor_sig, key, &enckey, msg, NULL, NULL);
        VALGRIND_MAKE_MEM_DEFINED(adaptor_sig, sizeof(adaptor_sig));
        VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
        CHECK(ret == 1);

        VALGRIND_MAKE_MEM_UNDEFINED(deckey, 32);
        ret = rustsecp256k1zkp_v0_4_0_ecdsa_adaptor_decrypt(ctx, &signature, deckey, adaptor_sig);
        VALGRIND_MAKE_MEM_DEFINED(&ret, sizeof(ret));
        CHECK(ret == 1);

        VALGRIND_MAKE_MEM_UNDEFINED(&signature, 32);
        ret = rustsecp256k1zkp_v0_4_0_ecdsa_adaptor_recover(ctx, expected_deckey, &signature, adaptor_sig, &enckey);
