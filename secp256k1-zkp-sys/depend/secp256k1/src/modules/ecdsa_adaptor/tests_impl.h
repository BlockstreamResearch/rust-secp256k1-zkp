...
void dleq_tests(void) {
    ...
    CHECK(rustsecp256k1zkp_v0_6_0_dleq_verify(&ctx->ecmult_ctx, &s, &e, &p1, &gen2, &p2) == 1);

    {
        rustsecp256k1zkp_v0_6_0_scalar tmp;
        rustsecp256k1zkp_v0_6_0_scalar_set_int(&tmp, 1);
        CHECK(rustsecp256k1zkp_v0_6_0_dleq_verify(&ctx->ecmult_ctx, &tmp, &e, &p1, &gen2, &p2) == 0);
        CHECK(rustsecp256k1zkp_v0_6_0_dleq_verify(&ctx->ecmult_ctx, &s, &tmp, &p1, &gen2, &p2) == 0);
    }
    {
        rustsecp256k1zkp_v0_6_0_ge p_tmp;
        rand_point(&p_tmp);
        CHECK(rustsecp256k1zkp_v0_6_0_dleq_verify(&ctx->ecmult_ctx, &s, &e, &p_tmp, &gen2, &p2) == 0);
        CHECK(rustsecp256k1zkp_v0_6_0_dleq_verify(&ctx->ecmult_ctx, &s, &e, &p1, &p_tmp, &p2) == 0);
        CHECK(rustsecp256k1zkp_v0_6_0_dleq_verify(&ctx->ecmult_ctx, &s, &e, &p1, &gen2, &p_tmp) == 0);
    }
    {
    ...
    rustsecp256k1zkp_v0_6_0_rfc6979_hmac_sha256_generate(&rustsecp256k1zkp_v0_6_0_test_rng, sk32, sizeof(sk32));
    rustsecp256k1zkp_v0_6_0_rfc6979_hmac_sha256_generate(&rustsecp256k1zkp_v0_6_0_test_rng, gen2_33, sizeof(gen2_33));
    rustsecp256k1zkp_v0_6_0_rfc6979_hmac_sha256_generate(&rustsecp256k1zkp_v0_6_0_test_rng, p1_33, sizeof(p1_33));
    rustsecp256k1zkp_v0_6_0_rfc6979_hmac_sha256_generate(&rustsecp256k1zkp_v0_6_0_test_rng, p2_33, sizeof(p2_33));
    rustsecp256k1zkp_v0_6_0_rfc6979_hmac_sha256_generate(&rustsecp256k1zkp_v0_6_0_test_rng, aux_rand, sizeof(aux_rand));
...
void run_nonce_function_ecdsa_adaptor_tests(void) {
    ...
    * state. */
    ...
    rustsecp256k1zkp_v0_6_0_rfc6979_hmac_sha256_generate(&rustsecp256k1zkp_v0_6_0_test_rng, msg, sizeof(msg));
    rustsecp256k1zkp_v0_6_0_rfc6979_hmac_sha256_generate(&rustsecp256k1zkp_v0_6_0_test_rng, key, sizeof(key));
    rustsecp256k1zkp_v0_6_0_rfc6979_hmac_sha256_generate(&rustsecp256k1zkp_v0_6_0_test_rng, pk, sizeof(pk));
    rustsecp256k1zkp_v0_6_0_rfc6979_hmac_sha256_generate(&rustsecp256k1zkp_v0_6_0_test_rng, aux_rand, sizeof(aux_rand));
...
void test_ecdsa_adaptor_api(void) {
    ...
    /** main test body **/
    ecount = 0;
    CHECK(rustsecp256k1zkp_v0_6_0_ecdsa_adaptor_encrypt(none, asig, sk, &enckey, msg, NULL, NULL) == 0);
    CHECK(ecount == 1);
    CHECK(rustsecp256k1zkp_v0_6_0_ecdsa_adaptor_encrypt(vrfy, asig, sk, &enckey, msg, NULL, NULL) == 0);
    CHECK(ecount == 2);
    ...
    ecount = 0;
    CHECK(rustsecp256k1zkp_v0_6_0_ecdsa_adaptor_encrypt(sign, asig, sk, &enckey, msg, NULL, NULL) == 1);
    CHECK(rustsecp256k1zkp_v0_6_0_ecdsa_adaptor_verify(none, asig, &pubkey, msg, &enckey) == 0);
    CHECK(ecount == 1);
    CHECK(rustsecp256k1zkp_v0_6_0_ecdsa_adaptor_verify(sign, asig, &pubkey, msg, &enckey) == 0);
    CHECK(ecount == 2);
    ...
    ecount = 0;
    CHECK(rustsecp256k1zkp_v0_6_0_ecdsa_adaptor_decrypt(both, &sig, deckey, asig) == 1);
    CHECK(rustsecp256k1zkp_v0_6_0_ecdsa_adaptor_recover(none, deckey, &sig, asig, &enckey) == 0);
    CHECK(ecount == 1);
    CHECK(rustsecp256k1zkp_v0_6_0_ecdsa_adaptor_recover(vrfy, deckey, &sig, asig, &enckey) == 0);
    CHECK(ecount == 2);
...
void adaptor_tests(void) {
    ...
    {
        /* Test key recover */
        rustsecp256k1zkp_v0_6_0_ecdsa_signature sig_tmp;
        ...
        CHECK(rustsecp256k1zkp_v0_6_0_ecdsa_adaptor_recover(ctx, decryption_key_tmp, &sig_tmp, adaptor_sig, &enckey) == 0);
    }
}
...
void multi_hop_lock_tests(void) {
    ...
    CHECK(rustsecp256k1zkp_v0_6_0_eckey_pubkey_tweak_add(&ctx->ecmult_ctx, &l_ge, &t1));
    rustsecp256k1zkp_v0_6_0_pubkey_save(&l, &l_ge);
    /* Right lock */
    rustsecp256k1zkp_v0_6_0_pubkey_load(ctx, &r_ge, &pubkey_pop);
    CHECK(rustsecp256k1zkp_v0_6_0_eckey_pubkey_tweak_add(&ctx->ecmult_ctx, &r_ge, &tp));
