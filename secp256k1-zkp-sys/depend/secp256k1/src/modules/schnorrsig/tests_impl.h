...
void test_schnorrsig_api(void) {
    ...
    /** main test body **/
    ecount = 0;
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(none, sig, msg, &keypairs[0], NULL, NULL) == 0);
    CHECK(ecount == 1);
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(vrfy, sig, msg, &keypairs[0], NULL, NULL) == 0);
    CHECK(ecount == 2);
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(sign, sig, msg, &keypairs[0], NULL, NULL) == 1);
    CHECK(ecount == 2);
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(sign, NULL, msg, &keypairs[0], NULL, NULL) == 0);
    CHECK(ecount == 3);
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(sign, sig, NULL, &keypairs[0], NULL, NULL) == 0);
    CHECK(ecount == 4);
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(sign, sig, msg, NULL, NULL, NULL) == 0);
    CHECK(ecount == 5);
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(sign, sig, msg, &invalid_keypair, NULL, NULL) == 0);
    CHECK(ecount == 6);

    ecount = 0;
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(sign, sig, msg, &keypairs[0], NULL, NULL) == 1);
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_verify(none, sig, msg, &pk[0]) == 0);
    CHECK(ecount == 1);
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_verify(sign, sig, msg, &pk[0]) == 0);
    CHECK(ecount == 2);
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_verify(vrfy, sig, msg, &pk[0]) == 1);
    CHECK(ecount == 2);
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_verify(vrfy, NULL, msg, &pk[0]) == 0);
    CHECK(ecount == 3);
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_verify(vrfy, sig, NULL, &pk[0]) == 0);
    CHECK(ecount == 4);
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_verify(vrfy, sig, msg, NULL) == 0);
    CHECK(ecount == 5);
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_verify(vrfy, sig, msg, &zero_pk) == 0);
    CHECK(ecount == 6);
...
void test_schnorrsig_bip_vectors_check_signing(const unsigned char *sk, const unsigned char *pk_serialized, unsigned char *aux_rand, const unsigned char *msg, const unsigned char *expected_sig) {
    ...
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(ctx, sig, msg, &keypair, NULL, aux_rand));
    ...
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_verify(ctx, sig, msg, &pk));
}
...
void test_schnorrsig_bip_vectors_check_verify(const unsigned char *pk_serialized, const unsigned char *msg32, const unsigned char *sig, int expected) {
    ...
    CHECK(expected == rustsecp256k1zkp_v0_6_0_schnorrsig_verify(ctx, sig, msg32, &pk));
}
...
void test_schnorrsig_sign(void) {
    ...
    CHECK(rustsecp256k1zkp_v0_6_0_keypair_create(ctx, &keypair, sk));
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(ctx, sig, msg, &keypair, NULL, NULL) == 1);

    /* Test different nonce functions */
    memset(sig, 1, sizeof(sig));
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(ctx, sig, msg, &keypair, nonce_function_failing, NULL) == 0);
    CHECK(rustsecp256k1zkp_v0_6_0_memcmp_var(sig, zeros64, sizeof(sig)) == 0);
    memset(&sig, 1, sizeof(sig));
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(ctx, sig, msg, &keypair, nonce_function_0, NULL) == 0);
    CHECK(rustsecp256k1zkp_v0_6_0_memcmp_var(sig, zeros64, sizeof(sig)) == 0);
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(ctx, sig, msg, &keypair, nonce_function_overflowing, NULL) == 1);
    CHECK(rustsecp256k1zkp_v0_6_0_memcmp_var(sig, zeros64, sizeof(sig)) != 0);
}
...
void test_schnorrsig_sign_verify(void) {
    ...
    for (i = 0; i < N_SIGS; i++) {
        ...
        CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(ctx, sig[i], msg[i], &keypair, NULL, NULL));
        CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_verify(ctx, sig[i], msg[i], &pk));
    }

    {
        ...
        size_t byte_idx = rustsecp256k1zkp_v0_6_0_testrand_int(32);
        unsigned char xorbyte = rustsecp256k1zkp_v0_6_0_testrand_int(254)+1;
        sig[sig_idx][byte_idx] ^= xorbyte;
        CHECK(!rustsecp256k1zkp_v0_6_0_schnorrsig_verify(ctx, sig[sig_idx], msg[sig_idx], &pk));
        sig[sig_idx][byte_idx] ^= xorbyte;

        byte_idx = rustsecp256k1zkp_v0_6_0_testrand_int(32);
    ...
    /* Test overflowing s */
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_sign(ctx, sig[0], msg[0], &keypair, NULL, NULL));
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_verify(ctx, sig[0], msg[0], &pk));
...
void test_schnorrsig_taproot(void) {
    ...
    CHECK(rustsecp256k1zkp_v0_6_0_schnorrsig_verify(ctx, sig, msg, &output_pk) == 1);
