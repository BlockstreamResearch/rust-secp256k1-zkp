...
static void test_generator_api(void) {
    ...
    CHECK(rustsecp256k1zkp_v0_11_0_generator_generate(CTX, &gen, key) == 1);
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_generator_generate(CTX, NULL, key));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_generator_generate(CTX, &gen, NULL));

    CHECK(rustsecp256k1zkp_v0_11_0_generator_generate_blinded(CTX, &gen, key, blind) == 1);
    CHECK_ILLEGAL(STATIC_CTX, rustsecp256k1zkp_v0_11_0_generator_generate_blinded(STATIC_CTX, &gen, key, blind));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_generator_generate_blinded(CTX, NULL, key, blind));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_generator_generate_blinded(CTX, &gen, NULL, blind));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_generator_generate_blinded(CTX, &gen, key, NULL));

    CHECK(rustsecp256k1zkp_v0_11_0_generator_serialize(CTX, sergen, &gen) == 1);
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_generator_serialize(CTX, NULL, &gen));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_generator_serialize(CTX, sergen, NULL));

    CHECK(rustsecp256k1zkp_v0_11_0_generator_serialize(CTX, sergen, &gen) == 1);
    CHECK(rustsecp256k1zkp_v0_11_0_generator_parse(CTX, &gen, sergen) == 1);
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_generator_parse(CTX, NULL, sergen));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_generator_parse(CTX, &gen, NULL));
}

static void test_shallue_van_de_woestijne(void) {
    /* Matches with the output of the shallue_van_de_woestijne.sage SAGE program */
    static const rustsecp256k1zkp_v0_11_0_ge_storage results[34] = {
    ...
    rustsecp256k1zkp_v0_11_0_ge_storage ges;
    int i, s;
    for (i = 0; i <= 16; i++) {
        rustsecp256k1zkp_v0_11_0_fe_set_int(&fe, i);

        for (s = 0; s < 2; s++) {
            if (s) {
                rustsecp256k1zkp_v0_11_0_fe_negate(&fe, &fe, 1);
                rustsecp256k1zkp_v0_11_0_fe_normalize(&fe);
            }
            shallue_van_de_woestijne(&ge, &fe);
            CHECK(rustsecp256k1zkp_v0_11_0_ge_is_valid_var(&ge));
            rustsecp256k1zkp_v0_11_0_ge_to_storage(&ges, &ge);
            CHECK(rustsecp256k1zkp_v0_11_0_memcmp_var(&ges, &results[i * 2 + s], sizeof(rustsecp256k1zkp_v0_11_0_ge_storage)) == 0);
        }
    }
}

static void test_generator_generate(void) {
    static const rustsecp256k1zkp_v0_11_0_ge_storage results[32] = {
    ...
    rustsecp256k1zkp_v0_11_0_scalar_set_b32(&sc, s, NULL);
    for (i = 1; i <= 32; i++) {
        memset(v, 0, 31);
        v[31] = i;
        CHECK(rustsecp256k1zkp_v0_11_0_generator_generate_blinded(CTX, &gen, v, s));
        rustsecp256k1zkp_v0_11_0_generator_load(&ge, &gen);
        rustsecp256k1zkp_v0_11_0_ge_to_storage(&ges, &ge);
        CHECK(rustsecp256k1zkp_v0_11_0_memcmp_var(&ges, &results[i - 1], sizeof(rustsecp256k1zkp_v0_11_0_ge_storage)) == 0);
        CHECK(rustsecp256k1zkp_v0_11_0_generator_generate(CTX, &gen, v));
        rustsecp256k1zkp_v0_11_0_generator_load(&ge, &gen);
        rustsecp256k1zkp_v0_11_0_ge_to_storage(&ges, &ge);
        CHECK(rustsecp256k1zkp_v0_11_0_memcmp_var(&ges, &results[i - 1], sizeof(rustsecp256k1zkp_v0_11_0_ge_storage)) == 0);
    }
...
static void test_generator_fixed_vector(void) {
    ...
    rustsecp256k1zkp_v0_11_0_generator parse;

    CHECK(rustsecp256k1zkp_v0_11_0_generator_parse(CTX, &parse, two_g));
    CHECK(rustsecp256k1zkp_v0_11_0_generator_serialize(CTX, result, &parse));
    CHECK(rustsecp256k1zkp_v0_11_0_memcmp_var(two_g, result, 33) == 0);

    result[0] = 0x0a;
    CHECK(rustsecp256k1zkp_v0_11_0_generator_parse(CTX, &parse, result));
    result[0] = 0x08;
    CHECK(!rustsecp256k1zkp_v0_11_0_generator_parse(CTX, &parse, result));
}
...
static void test_pedersen_api(void) {
    ...
    const rustsecp256k1zkp_v0_11_0_pedersen_commitment *commit_ptr = &commit;
    unsigned char blind[32];
    ...
    testrand256(blind);
    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_commit(CTX, &commit, blind, val, rustsecp256k1zkp_v0_11_0_generator_h) != 0);
    CHECK_ILLEGAL(STATIC_CTX, rustsecp256k1zkp_v0_11_0_pedersen_commit(STATIC_CTX, &commit, blind, val, rustsecp256k1zkp_v0_11_0_generator_h));

    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_commit(CTX, NULL, blind, val, rustsecp256k1zkp_v0_11_0_generator_h));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_commit(CTX, &commit, NULL, val, rustsecp256k1zkp_v0_11_0_generator_h));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_commit(CTX, &commit, blind, val, NULL));

    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_blind_sum(CTX, blind_out, &blind_ptr, 1, 1) != 0);
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_blind_sum(CTX, NULL, &blind_ptr, 1, 1));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_blind_sum(CTX, blind_out, NULL, 1, 1));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_blind_sum(CTX, blind_out, &blind_ptr, 0, 1));
    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_blind_sum(CTX, blind_out, &blind_ptr, 0, 0) != 0);

    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_commit(CTX, &commit, blind, val, rustsecp256k1zkp_v0_11_0_generator_h) != 0);
    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_verify_tally(CTX, &commit_ptr, 1, &commit_ptr, 1) != 0);
    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_verify_tally(CTX, NULL, 0, &commit_ptr, 1) == 0);
    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_verify_tally(CTX, &commit_ptr, 1, NULL, 0) == 0);
    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_verify_tally(CTX, NULL, 0, NULL, 0) != 0);
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_verify_tally(CTX, NULL, 1, &commit_ptr, 1));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_verify_tally(CTX, &commit_ptr, 1, NULL, 1));

    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_blind_generator_blind_sum(CTX, &val, &blind_ptr, &blind_out_ptr, 1, 0) != 0);
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_blind_generator_blind_sum(CTX, &val, &blind_ptr, &blind_out_ptr, 1, 1));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_blind_generator_blind_sum(CTX, &val, &blind_ptr, &blind_out_ptr, 0, 0));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_blind_generator_blind_sum(CTX, NULL, &blind_ptr, &blind_out_ptr, 1, 0));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_blind_generator_blind_sum(CTX, &val, NULL, &blind_out_ptr, 1, 0));
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_blind_generator_blind_sum(CTX, &val, &blind_ptr, NULL, 1, 0));
    /* check that NULL in array of generator_blind pointers is not allowed */
    blind_ptr = NULL;
    CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_blind_generator_blind_sum(CTX, &val, &blind_ptr, &blind_out_ptr, 1, 0));
    blind_ptr = blind;
...
static void test_pedersen_internal(void) {
    rustsecp256k1zkp_v0_11_0_pedersen_commitment commits[19];
    const rustsecp256k1zkp_v0_11_0_pedersen_commitment *cptr[19];
    ...
    for (i = 0; i < total - 1; i++) {
        ...
        rustsecp256k1zkp_v0_11_0_scalar_get_b32(&blinds[i * 32], &s);
    }
    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_blind_sum(CTX, &blinds[(total - 1) * 32], bptr, total - 1, inputs));
    /* check that NULL in array of blind pointers is not allowed */
    for (i = 0; i < total - 1; i++) {
        unsigned char blind_out[32];
        const unsigned char *original_ptr = bptr[i];
        bptr[i] = NULL;
        CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_blind_sum(CTX, blind_out, bptr, total - 1, inputs));
        bptr[i] = original_ptr;
    }
    for (i = 0; i < total; i++) {
        unsigned char result[33];
        rustsecp256k1zkp_v0_11_0_pedersen_commitment parse;

        CHECK(rustsecp256k1zkp_v0_11_0_pedersen_commit(CTX, &commits[i], &blinds[i * 32], values[i], rustsecp256k1zkp_v0_11_0_generator_h));
        CHECK(rustsecp256k1zkp_v0_11_0_pedersen_commitment_serialize(CTX, result, &commits[i]));
        CHECK(rustsecp256k1zkp_v0_11_0_pedersen_commitment_parse(CTX, &parse, result));
        CHECK(rustsecp256k1zkp_v0_11_0_memcmp_var(&commits[i], &parse, 33) == 0);
    }
    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_verify_tally(CTX, cptr, inputs, &cptr[inputs], outputs));
    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_verify_tally(CTX, &cptr[inputs], outputs, cptr, inputs));
    /* check that NULL in array of commits pointers is not allowed */
    for (i = 0; i < inputs; i++) {
        const rustsecp256k1zkp_v0_11_0_pedersen_commitment *original_ptr = cptr[i];
        cptr[i] = NULL;
        CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_verify_tally(CTX, cptr, inputs, &cptr[inputs], outputs));
        cptr[i] = original_ptr;
    }
    /* check that NULL in array of ncommits pointers is not allowed */
    for (i = 0; i < outputs; i++) {
        const rustsecp256k1zkp_v0_11_0_pedersen_commitment *original_ptr = cptr[inputs + i];
        cptr[inputs + i] = NULL;
        CHECK_ILLEGAL(CTX, rustsecp256k1zkp_v0_11_0_pedersen_verify_tally(CTX, cptr, inputs, &cptr[inputs], outputs));
        cptr[inputs + i] = original_ptr;
    }
    if (inputs > 0 && values[0] > 0) {
        CHECK(!rustsecp256k1zkp_v0_11_0_pedersen_verify_tally(CTX, cptr, inputs - 1, &cptr[inputs], outputs));
    }
    testutil_random_scalar_order(&s);
    for (i = 0; i < 4; i++) {
        rustsecp256k1zkp_v0_11_0_scalar_get_b32(&blinds[i * 32], &s);
    ...
    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_verify_tally(CTX, &cptr[0], 1, &cptr[0], 1));
    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_verify_tally(CTX, &cptr[1], 1, &cptr[1], 1));
}
...
static void test_pedersen_commitment_fixed_vector(void) {
    ...
    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_commitment_parse(CTX, &parse, two_g));
    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_commitment_serialize(CTX, result, &parse));
    CHECK(rustsecp256k1zkp_v0_11_0_memcmp_var(two_g, result, 33) == 0);
