...
static void test_whitelist_end_to_end_internal(const unsigned char *summed_seckey, const unsigned char *online_seckey, const rustsecp256k1zkp_v0_11_0_pubkey *online_pubkeys, const rustsecp256k1zkp_v0_11_0_pubkey *offline_pubkeys, const rustsecp256k1zkp_v0_11_0_pubkey *sub_pubkey, const size_t signer_i, const size_t n_keys) {
        unsigned char serialized[32 + 4 + 32 * SECP256K1_WHITELIST_MAX_N_KEYS] = {0};
        size_t slen = sizeof(serialized);
        rustsecp256k1zkp_v0_11_0_whitelist_signature sig;
        rustsecp256k1zkp_v0_11_0_whitelist_signature sig1;

        CHECK(rustsecp256k1zkp_v0_11_0_whitelist_sign(CTX, &sig, online_pubkeys, offline_pubkeys, n_keys, sub_pubkey, online_seckey, summed_seckey, signer_i));
        CHECK(rustsecp256k1zkp_v0_11_0_whitelist_verify(CTX, &sig, online_pubkeys, offline_pubkeys, n_keys, sub_pubkey) == 1);
        /* Check that exchanging keys causes a failure */
        CHECK(rustsecp256k1zkp_v0_11_0_whitelist_verify(CTX, &sig, offline_pubkeys, online_pubkeys, n_keys, sub_pubkey) != 1);
        ...
        /* Test n_keys */
        CHECK(rustsecp256k1zkp_v0_11_0_whitelist_signature_n_keys(&sig) == n_keys);
        CHECK(rustsecp256k1zkp_v0_11_0_whitelist_signature_n_keys(&sig1) == n_keys);
...
static void test_whitelist_end_to_end(const size_t n_keys, int test_all_keys) {
    ...
    rustsecp256k1zkp_v0_11_0_pubkey *online_pubkeys = malloc(n_keys * sizeof(*online_pubkeys));
    rustsecp256k1zkp_v0_11_0_pubkey *offline_pubkeys = malloc(n_keys * sizeof(*offline_pubkeys));

    rustsecp256k1zkp_v0_11_0_scalar ssub;
    unsigned char csub[32];
    rustsecp256k1zkp_v0_11_0_pubkey sub_pubkey;

    /* Generate random keys */
    size_t i;
    /* Start with subkey */
    testutil_random_scalar_order_test(&ssub);
    rustsecp256k1zkp_v0_11_0_scalar_get_b32(csub, &ssub);
    CHECK(rustsecp256k1zkp_v0_11_0_ec_seckey_verify(CTX, csub) == 1);
    CHECK(rustsecp256k1zkp_v0_11_0_ec_pubkey_create(CTX, &sub_pubkey, csub) == 1);
    /* Then offline and online whitelist keys */
    for (i = 0; i < n_keys; i++) {
        rustsecp256k1zkp_v0_11_0_scalar son, soff;
        ...
        rustsecp256k1zkp_v0_11_0_scalar_get_b32(online_seckey[i], &son);
        CHECK(rustsecp256k1zkp_v0_11_0_ec_seckey_verify(CTX, online_seckey[i]) == 1);
        CHECK(rustsecp256k1zkp_v0_11_0_ec_pubkey_create(CTX, &online_pubkeys[i], online_seckey[i]) == 1);

        testutil_random_scalar_order_test(&soff);
        rustsecp256k1zkp_v0_11_0_scalar_get_b32(summed_seckey[i], &soff);
        CHECK(rustsecp256k1zkp_v0_11_0_ec_seckey_verify(CTX, summed_seckey[i]) == 1);
        CHECK(rustsecp256k1zkp_v0_11_0_ec_pubkey_create(CTX, &offline_pubkeys[i], summed_seckey[i]) == 1);

        /* Make summed_seckey correspond to the sum of offline_pubkey and sub_pubkey */
        rustsecp256k1zkp_v0_11_0_scalar_add(&soff, &soff, &ssub);
        rustsecp256k1zkp_v0_11_0_scalar_get_b32(summed_seckey[i], &soff);
        CHECK(rustsecp256k1zkp_v0_11_0_ec_seckey_verify(CTX, summed_seckey[i]) == 1);
    }
...
static void test_whitelist_bad_parse(void) {
    ...
    /* Empty input */
    CHECK(rustsecp256k1zkp_v0_11_0_whitelist_signature_parse(CTX, &sig, serialized0, 0) == 0);
    /* Misses one byte of e0 */
    CHECK(rustsecp256k1zkp_v0_11_0_whitelist_signature_parse(CTX, &sig, serialized1, sizeof(serialized1)) == 0);
    /* Enough bytes for e0, but there is no s value */
    CHECK(rustsecp256k1zkp_v0_11_0_whitelist_signature_parse(CTX, &sig, serialized2, sizeof(serialized2)) == 0);
}
...
static void test_whitelist_bad_serialize(void) {
    ...
    CHECK(rustsecp256k1zkp_v0_11_0_whitelist_signature_parse(CTX, &sig, serialized, sizeof(serialized)) == 1);
    serialized_len = sizeof(serialized) - 1;
    /* Output buffer is one byte too short */
    CHECK(rustsecp256k1zkp_v0_11_0_whitelist_signature_serialize(CTX, serialized, &serialized_len, &sig) == 0);
}
