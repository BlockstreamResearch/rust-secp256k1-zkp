...
static void rustsecp256k1zkp_v0_10_0_keyagg_cache_save(rustsecp256k1zkp_v0_10_0_musig_keyagg_cache *cache, rustsecp256k1zkp_v0_10_0_keyagg_cache_internal *cache_i) {
    unsigned char *ptr = cache->data;
    memcpy(ptr, rustsecp256k1zkp_v0_10_0_musig_keyagg_cache_magic, 4);
    ptr += 4;
...
static int rustsecp256k1zkp_v0_10_0_keyagg_cache_load(const rustsecp256k1zkp_v0_10_0_context* ctx, rustsecp256k1zkp_v0_10_0_keyagg_cache_internal *cache_i, const rustsecp256k1zkp_v0_10_0_musig_keyagg_cache *cache) {
    ...
    ARG_CHECK(rustsecp256k1zkp_v0_10_0_memcmp_var(ptr, rustsecp256k1zkp_v0_10_0_musig_keyagg_cache_magic, 4) == 0);
    ptr += 4;
...
static void rustsecp256k1zkp_v0_10_0_musig_keyagglist_sha256(rustsecp256k1zkp_v0_10_0_sha256 *sha) {
...
static int rustsecp256k1zkp_v0_10_0_musig_compute_pk_hash(const rustsecp256k1zkp_v0_10_0_context *ctx, unsigned char *pk_hash, const rustsecp256k1zkp_v0_10_0_pubkey * const* pk, size_t np) {
    rustsecp256k1zkp_v0_10_0_sha256 sha;
    size_t i;

    rustsecp256k1zkp_v0_10_0_musig_keyagglist_sha256(&sha);
    for (i = 0; i < np; i++) {
        unsigned char ser[33];
        size_t ser_len = sizeof(ser);
        if (!rustsecp256k1zkp_v0_10_0_ec_pubkey_serialize(ctx, ser, &ser_len, pk[i], SECP256K1_EC_COMPRESSED)) {
            return 0;
        }
        VERIFY_CHECK(ser_len == sizeof(ser));
        rustsecp256k1zkp_v0_10_0_sha256_write(&sha, ser, sizeof(ser));
    }
    rustsecp256k1zkp_v0_10_0_sha256_finalize(&sha, pk_hash);
    return 1;
}

/* Initializes SHA256 with fixed midstate. This midstate was computed by applying
 * SHA256 to SHA256("KeyAgg coefficient")||SHA256("KeyAgg coefficient"). */
static void rustsecp256k1zkp_v0_10_0_musig_keyaggcoef_sha256(rustsecp256k1zkp_v0_10_0_sha256 *sha) {
    rustsecp256k1zkp_v0_10_0_sha256_initialize(sha);
...
static void rustsecp256k1zkp_v0_10_0_musig_keyaggcoef_internal(rustsecp256k1zkp_v0_10_0_scalar *r, const unsigned char *pk_hash, rustsecp256k1zkp_v0_10_0_ge *pk, const rustsecp256k1zkp_v0_10_0_ge *second_pk) {
    rustsecp256k1zkp_v0_10_0_sha256 sha;

    VERIFY_CHECK(!rustsecp256k1zkp_v0_10_0_ge_is_infinity(pk));

    if (!rustsecp256k1zkp_v0_10_0_ge_is_infinity(second_pk)
          && rustsecp256k1zkp_v0_10_0_fe_equal(&pk->x, &second_pk->x)
          && rustsecp256k1zkp_v0_10_0_fe_is_odd(&pk->y) == rustsecp256k1zkp_v0_10_0_fe_is_odd(&second_pk->y)) {
        rustsecp256k1zkp_v0_10_0_scalar_set_int(r, 1);
    } else {
        unsigned char buf[33];
        size_t buflen = sizeof(buf);
        int ret;
        rustsecp256k1zkp_v0_10_0_musig_keyaggcoef_sha256(&sha);
        rustsecp256k1zkp_v0_10_0_sha256_write(&sha, pk_hash, 32);
        ret = rustsecp256k1zkp_v0_10_0_eckey_pubkey_serialize(pk, buf, &buflen, 1);
#ifdef VERIFY
        ...
        rustsecp256k1zkp_v0_10_0_scalar_set_b32(r, buf, NULL);
    }
}
...
static void rustsecp256k1zkp_v0_10_0_musig_keyaggcoef(rustsecp256k1zkp_v0_10_0_scalar *r, const rustsecp256k1zkp_v0_10_0_keyagg_cache_internal *cache_i, rustsecp256k1zkp_v0_10_0_ge *pk) {
    rustsecp256k1zkp_v0_10_0_musig_keyaggcoef_internal(r, cache_i->pk_hash, pk, &cache_i->second_pk);
}
...
static int rustsecp256k1zkp_v0_10_0_musig_pubkey_agg_callback(rustsecp256k1zkp_v0_10_0_scalar *sc, rustsecp256k1zkp_v0_10_0_ge *pt, size_t idx, void *data) {
    rustsecp256k1zkp_v0_10_0_musig_pubkey_agg_ecmult_data *ctx = (rustsecp256k1zkp_v0_10_0_musig_pubkey_agg_ecmult_data *) data;
    int ret;
    ret = rustsecp256k1zkp_v0_10_0_pubkey_load(ctx->ctx, pt, ctx->pks[idx]);
#ifdef VERIFY
    ...
    (void) ret;
#endif
    rustsecp256k1zkp_v0_10_0_musig_keyaggcoef_internal(sc, ctx->pk_hash, pt, &ctx->second_pk);
    return 1;
}

int rustsecp256k1zkp_v0_10_0_musig_pubkey_agg(const rustsecp256k1zkp_v0_10_0_context* ctx, rustsecp256k1zkp_v0_10_0_scratch_space *scratch, rustsecp256k1zkp_v0_10_0_xonly_pubkey *agg_pk, rustsecp256k1zkp_v0_10_0_musig_keyagg_cache *keyagg_cache, const rustsecp256k1zkp_v0_10_0_pubkey * const* pubkeys, size_t n_pubkeys) {
    rustsecp256k1zkp_v0_10_0_musig_pubkey_agg_ecmult_data ecmult_data;
    ...
    rustsecp256k1zkp_v0_10_0_ge_set_infinity(&ecmult_data.second_pk);
    for (i = 1; i < n_pubkeys; i++) {
        if (rustsecp256k1zkp_v0_10_0_memcmp_var(pubkeys[0], pubkeys[i], sizeof(*pubkeys[0])) != 0) {
            rustsecp256k1zkp_v0_10_0_ge pk;
            if (!rustsecp256k1zkp_v0_10_0_pubkey_load(ctx, &pk, pubkeys[i])) {
                return 0;
            }
    ...
    if (!rustsecp256k1zkp_v0_10_0_musig_compute_pk_hash(ctx, ecmult_data.pk_hash, pubkeys, n_pubkeys)) {
        return 0;
    }
    /* TODO: actually use optimized ecmult_multi algorithms by providing a
     * scratch space */
    if (!rustsecp256k1zkp_v0_10_0_ecmult_multi_var(&ctx->error_callback, NULL, &pkj, NULL, rustsecp256k1zkp_v0_10_0_musig_pubkey_agg_callback, (void *) &ecmult_data, n_pubkeys)) {
    ...
    rustsecp256k1zkp_v0_10_0_ge_set_gej(&pkp, &pkj);
    rustsecp256k1zkp_v0_10_0_fe_normalize_var(&pkp.y);
    /* The resulting public key is infinity with negligible probability */
    VERIFY_CHECK(!rustsecp256k1zkp_v0_10_0_ge_is_infinity(&pkp));
    if (keyagg_cache != NULL) {
        rustsecp256k1zkp_v0_10_0_keyagg_cache_internal cache_i = { 0 };
        cache_i.pk = pkp;
    ...
    rustsecp256k1zkp_v0_10_0_extrakeys_ge_even_y(&pkp);
    if (agg_pk != NULL) {
        rustsecp256k1zkp_v0_10_0_xonly_pubkey_save(agg_pk, &pkp);
    }
    return 1;
}

int rustsecp256k1zkp_v0_10_0_musig_pubkey_get(const rustsecp256k1zkp_v0_10_0_context* ctx, rustsecp256k1zkp_v0_10_0_pubkey *agg_pk, const rustsecp256k1zkp_v0_10_0_musig_keyagg_cache *keyagg_cache) {
...
static int rustsecp256k1zkp_v0_10_0_musig_pubkey_tweak_add_internal(const rustsecp256k1zkp_v0_10_0_context* ctx, rustsecp256k1zkp_v0_10_0_pubkey *output_pubkey, rustsecp256k1zkp_v0_10_0_musig_keyagg_cache *keyagg_cache, const unsigned char *tweak32, int xonly) {
    rustsecp256k1zkp_v0_10_0_keyagg_cache_internal cache_i;
    int overflow = 0;
...
int rustsecp256k1zkp_v0_10_0_musig_pubkey_ec_tweak_add(const rustsecp256k1zkp_v0_10_0_context* ctx, rustsecp256k1zkp_v0_10_0_pubkey *output_pubkey, rustsecp256k1zkp_v0_10_0_musig_keyagg_cache *keyagg_cache, const unsigned char *tweak32) {
    return rustsecp256k1zkp_v0_10_0_musig_pubkey_tweak_add_internal(ctx, output_pubkey, keyagg_cache, tweak32, 0);
}

int rustsecp256k1zkp_v0_10_0_musig_pubkey_xonly_tweak_add(const rustsecp256k1zkp_v0_10_0_context* ctx, rustsecp256k1zkp_v0_10_0_pubkey *output_pubkey, rustsecp256k1zkp_v0_10_0_musig_keyagg_cache *keyagg_cache, const unsigned char *tweak32) {
    return rustsecp256k1zkp_v0_10_0_musig_pubkey_tweak_add_internal(ctx, output_pubkey, keyagg_cache, tweak32, 1);
}
