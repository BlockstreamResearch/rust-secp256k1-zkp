...
in the data structure `rustsecp256k1zkp_v0_2_0_musig_session`. The API does not expose any
...
for this data structure in `include/rustsecp256k1zkp_v0_2_0_musig.h`.
...
by calling `rustsecp256k1zkp_v0_2_0_musig_pubkey_combine`.
...
1. The signer starts the session by calling `rustsecp256k1zkp_v0_2_0_musig_session_init`.
   ...
   - the combined public key output from `rustsecp256k1zkp_v0_2_0_musig_pubkey_combine`
   - the public key hash output from `rustsecp256k1zkp_v0_2_0_musig_pubkey_combine`
...
3. Once all signers nonce commitments have been received, the signer records
   these commitments with the function `rustsecp256k1zkp_v0_2_0_musig_session_get_public_nonce`.
   If the signer did not provide a message to `rustsecp256k1zkp_v0_2_0_musig_session_init`,
...
4. The signer then communicates `R_i` to all other signers, and receives `R_j`
   ...
   `rustsecp256k1zkp_v0_2_0_musig_set_nonce` to record this fact. This function checks that
   ...
   `rustsecp256k1zkp_v0_2_0_ec_pubkey_serialize` and parsed with `rustsecp256k1zkp_v0_2_0_ec_pubkey_parse`.
5. Once all nonces have been exchanged in this way, signers are able to compute
   their partial signatures. They do so by calling `rustsecp256k1zkp_v0_2_0_musig_session_combine_nonces`
...
6. The signer computes a partial signature `s_i` using the function
   `rustsecp256k1zkp_v0_2_0_musig_partial_sign` which takes the session state as input and
...
8. Each signer calls `rustsecp256k1zkp_v0_2_0_musig_partial_sig_verify` on the other signers' partial
...
9. Any signer, or central coordinator, may combine the partial signatures to obtain
   a complete signature using `rustsecp256k1zkp_v0_2_0_musig_partial_sig_combine`. This function takes
...
2. The participant receives nonce commitments, public nonces and partial signatures,
   but does not produce these values. Therefore `rustsecp256k1zkp_v0_2_0_musig_session_get_public_nonce`
   and `rustsecp256k1zkp_v0_2_0_musig_partial_sign` are not called.

### Verifier

The final signature is simply a valid Schnorr signature using the combined public key. It
can be verified using the `rustsecp256k1zkp_v0_2_0_schnorrsig_verify` with the correct message and
public key output from `rustsecp256k1zkp_v0_2_0_musig_pubkey_combine`.
...
1. Before the protocol begins, Bob chooses a 32-byte auxiliary secret `t` at random and
   computes a corresponding public point `T` by calling `rustsecp256k1zkp_v0_2_0_ec_pubkey_create`.
...
3. At step 5, when combining the two parties' public nonces, both parties call
   `rustsecp256k1zkp_v0_2_0_musig_session_combine_nonces` with `adaptor` set to `T` and `nonce_is_negated`
...
5. In Session B, Bob calls `rustsecp256k1zkp_v0_2_0_musig_partial_sig_adapt` with his partial signature
   and `t`, to produce an adaptor signature. He can then call `rustsecp256k1zkp_v0_2_0_musig_partial_sig_combine`
   with this adaptor signature and Alice's partial signature, to produce a complete
   signature for blockchain B.
6. Alice reads this signature from blockchain B. She calls `rustsecp256k1zkp_v0_2_0_musig_extract_secret_adaptor`,
...
7. In Session A, Alice is now able to replicate Bob's action, calling
   `rustsecp256k1zkp_v0_2_0_musig_partial_sig_adapt` with her own partial signature and `t`, ultimately
