...
static size_t rustsecp256k1zkp_v0_10_0_count_bits_set(const unsigned char* data, size_t count) {
    size_t ret = 0;
    size_t i;
    for (i = 0; i < count; i++) {
...
static
#endif
int rustsecp256k1zkp_v0_10_0_surjectionproof_parse(const rustsecp256k1zkp_v0_10_0_context* ctx, rustsecp256k1zkp_v0_10_0_surjectionproof *proof, const unsigned char *input, size_t inputlen) {
    size_t n_inputs;
    ...
    signature_len = 32 * (1 + rustsecp256k1zkp_v0_10_0_count_bits_set(&input[2], (n_inputs + 7) / 8));
...
int rustsecp256k1zkp_v0_10_0_surjectionproof_serialize(const rustsecp256k1zkp_v0_10_0_context* ctx, unsigned char *output, size_t *outputlen, const rustsecp256k1zkp_v0_10_0_surjectionproof *proof) {
    size_t signature_len;
    ...
    signature_len = 32 * (1 + rustsecp256k1zkp_v0_10_0_count_bits_set(proof->used_inputs, (proof->n_inputs + 7) / 8));
...
size_t rustsecp256k1zkp_v0_10_0_surjectionproof_n_total_inputs(const rustsecp256k1zkp_v0_10_0_context* ctx, const rustsecp256k1zkp_v0_10_0_surjectionproof* proof) {
    VERIFY_CHECK(ctx != NULL);
    ARG_CHECK(proof != NULL);
    (void) ctx;
...
size_t rustsecp256k1zkp_v0_10_0_surjectionproof_n_used_inputs(const rustsecp256k1zkp_v0_10_0_context* ctx, const rustsecp256k1zkp_v0_10_0_surjectionproof* proof) {
    VERIFY_CHECK(ctx != NULL);
    ARG_CHECK(proof != NULL);
    (void) ctx;
    return rustsecp256k1zkp_v0_10_0_count_bits_set(proof->used_inputs, (proof->n_inputs + 7) / 8);
}

size_t rustsecp256k1zkp_v0_10_0_surjectionproof_serialized_size(const rustsecp256k1zkp_v0_10_0_context* ctx, const rustsecp256k1zkp_v0_10_0_surjectionproof* proof) {
    VERIFY_CHECK(ctx != NULL);
    ARG_CHECK(proof != NULL);
...
static void rustsecp256k1zkp_v0_10_0_surjectionproof_csprng_init(rustsecp256k1zkp_v0_10_0_surjectionproof_csprng *csprng, const unsigned char* state) {
    memcpy(csprng->state, state, 32);
    csprng->state_i = 0;
}

static size_t rustsecp256k1zkp_v0_10_0_surjectionproof_csprng_next(rustsecp256k1zkp_v0_10_0_surjectionproof_csprng *csprng, size_t rand_max) {
    ...
    while (1) {
        ...
        if (csprng->state_i + increment >= 32) {
            ...
            rustsecp256k1zkp_v0_10_0_sha256_write(&sha, csprng->state, 32);
            rustsecp256k1zkp_v0_10_0_sha256_finalize(&sha, csprng->state);
            csprng->state_i = 0;
        }
...
int rustsecp256k1zkp_v0_10_0_surjectionproof_initialize(const rustsecp256k1zkp_v0_10_0_context* ctx, rustsecp256k1zkp_v0_10_0_surjectionproof* proof, size_t *input_index, const rustsecp256k1zkp_v0_10_0_fixed_asset_tag* fixed_input_tags, const size_t n_input_tags, const size_t n_input_tags_to_use, const rustsecp256k1zkp_v0_10_0_fixed_asset_tag* fixed_output_tag, const size_t n_max_iterations, const unsigned char *random_seed32) {
    rustsecp256k1zkp_v0_10_0_surjectionproof_csprng csprng;
    ...
    rustsecp256k1zkp_v0_10_0_surjectionproof_csprng_init(&csprng, random_seed32);
    ...
    while (1) {
        ...
        for (i = 0; i < n_input_tags_to_use; i++) {
            while (1) {
                ...
                next_input_index = rustsecp256k1zkp_v0_10_0_surjectionproof_csprng_next(&csprng, n_input_tags);
                if (rustsecp256k1zkp_v0_10_0_memcmp_var(&fixed_input_tags[next_input_index], fixed_output_tag, sizeof(*fixed_output_tag)) == 0) {
...
int rustsecp256k1zkp_v0_10_0_surjectionproof_generate(const rustsecp256k1zkp_v0_10_0_context* ctx, rustsecp256k1zkp_v0_10_0_surjectionproof* proof, const rustsecp256k1zkp_v0_10_0_generator* ephemeral_input_tags, size_t n_ephemeral_input_tags, const rustsecp256k1zkp_v0_10_0_generator* ephemeral_output_tag, size_t input_index, const unsigned char *input_blinding_key, const unsigned char *output_blinding_key) {
    rustsecp256k1zkp_v0_10_0_scalar blinding_key;
    rustsecp256k1zkp_v0_10_0_scalar tmps;
    ...
    rustsecp256k1zkp_v0_10_0_gej ring_pubkeys[SECP256K1_SURJECTIONPROOF_MAX_USED_INPUTS];
    rustsecp256k1zkp_v0_10_0_scalar borromean_s[SECP256K1_SURJECTIONPROOF_MAX_USED_INPUTS];
    ...
    n_used_pubkeys = rustsecp256k1zkp_v0_10_0_surjectionproof_n_used_inputs(ctx, proof);
    /* This must be true if the proof was created with surjectionproof_initialize */
    ARG_CHECK(n_used_pubkeys > 0);

    /* Compute secret key */
    rustsecp256k1zkp_v0_10_0_scalar_set_b32(&tmps, input_blinding_key, &overflow);
    if (overflow) {
        return 0;
    }
    rustsecp256k1zkp_v0_10_0_scalar_set_b32(&blinding_key, output_blinding_key, &overflow);
    if (overflow) {
        return 0;
    }
    ...
    for (i = 0; i < n_ephemeral_input_tags; i++) {
        if (rustsecp256k1zkp_v0_10_0_memcmp_var(ephemeral_input_tags[i].data, ephemeral_output_tag->data, sizeof(ephemeral_output_tag->data)) == 0) {
            return 0;
        }
    }
    rustsecp256k1zkp_v0_10_0_scalar_negate(&tmps, &tmps);
    rustsecp256k1zkp_v0_10_0_scalar_add(&blinding_key, &blinding_key, &tmps);
    ...
    if (rustsecp256k1zkp_v0_10_0_surjection_compute_public_keys(ring_pubkeys, n_used_pubkeys, ephemeral_input_tags, n_total_pubkeys, proof->used_inputs, ephemeral_output_tag, input_index, &ring_input_index) == 0) {
        return 0;
    }
    ...
    rustsecp256k1zkp_v0_10_0_surjection_genmessage(msg32, ephemeral_input_tags, n_total_pubkeys, ephemeral_output_tag);
    if (rustsecp256k1zkp_v0_10_0_surjection_genrand(borromean_s, n_used_pubkeys, &blinding_key) == 0) {
        return 0;
    }
    ...
    rustsecp256k1zkp_v0_10_0_scalar_clear(&borromean_s[ring_input_index]);
    if (rustsecp256k1zkp_v0_10_0_borromean_sign(&ctx->ecmult_gen_ctx, &proof->data[0], borromean_s, ring_pubkeys, &nonce, &blinding_key, rsizes, indices, 1, msg32, 32) == 0) {
        return 0;
    }
    for (i = 0; i < n_used_pubkeys; i++) {
        rustsecp256k1zkp_v0_10_0_scalar_get_b32(&proof->data[32 + 32 * i], &borromean_s[i]);
    }
    return 1;
}
...
int rustsecp256k1zkp_v0_10_0_surjectionproof_verify(const rustsecp256k1zkp_v0_10_0_context* ctx, const rustsecp256k1zkp_v0_10_0_surjectionproof* proof, const rustsecp256k1zkp_v0_10_0_generator* ephemeral_input_tags, size_t n_ephemeral_input_tags, const rustsecp256k1zkp_v0_10_0_generator* ephemeral_output_tag) {
    ...
    if (rustsecp256k1zkp_v0_10_0_surjection_compute_public_keys(ring_pubkeys, n_used_pubkeys, ephemeral_input_tags, n_total_pubkeys, proof->used_inputs, ephemeral_output_tag, 0, NULL) == 0) {
        return 0;
    }
    ...
    for (i = 0; i < n_used_pubkeys; i++) {
        int overflow = 0;
        rustsecp256k1zkp_v0_10_0_scalar_set_b32(&borromean_s[i], &proof->data[32 + 32 * i], &overflow);
        if (overflow == 1) {
            return 0;
        }
    }
    rustsecp256k1zkp_v0_10_0_surjection_genmessage(msg32, ephemeral_input_tags, n_total_pubkeys, ephemeral_output_tag);
    return rustsecp256k1zkp_v0_10_0_borromean_verify(NULL, &proof->data[0], borromean_s, ring_pubkeys, rsizes, 1, msg32, 32);
}
