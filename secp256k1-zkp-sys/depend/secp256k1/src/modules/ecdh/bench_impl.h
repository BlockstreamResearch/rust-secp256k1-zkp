...
static void bench_ecdh_setup(void* arg) {
    ...
    CHECK(rustsecp256k1zkp_v0_11_0_ec_pubkey_parse(data->ctx, &data->point, point, sizeof(point)) == 1);
}
...
static void bench_ecdh(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        CHECK(rustsecp256k1zkp_v0_11_0_ecdh(data->ctx, res, &data->point, data->scalar, NULL, NULL) == 1);
    }
}
