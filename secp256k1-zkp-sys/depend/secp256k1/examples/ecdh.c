...
int main(void) {
    ...
    int return_val;
    size_t len;
    rustsecp256k1zkp_v0_7_0_pubkey pubkey1;
    rustsecp256k1zkp_v0_7_0_pubkey pubkey2;

    /* The specification in secp256k1.h states that `rustsecp256k1zkp_v0_7_0_ec_pubkey_create`
     * needs a context object initialized for signing, which is why we create
     * a context with the SECP256K1_CONTEXT_SIGN flag.
     * (The docs for `rustsecp256k1zkp_v0_7_0_ecdh` don't require any special context, just
     * some initialized context) */
    rustsecp256k1zkp_v0_7_0_context* ctx = rustsecp256k1zkp_v0_7_0_context_create(SECP256K1_CONTEXT_SIGN);
    ...
    while (1) {
        ...
        if (rustsecp256k1zkp_v0_7_0_ec_seckey_verify(ctx, seckey1) && rustsecp256k1zkp_v0_7_0_ec_seckey_verify(ctx, seckey2)) {
            break;
        }
    }

    /* Public key creation using a valid context with a verified secret key should never fail */
    return_val = rustsecp256k1zkp_v0_7_0_ec_pubkey_create(ctx, &pubkey1, seckey1);
    assert(return_val);
    return_val = rustsecp256k1zkp_v0_7_0_ec_pubkey_create(ctx, &pubkey2, seckey2);
    assert(return_val);

    /* Serialize pubkey1 in a compressed form (33 bytes), should always return 1 */
    len = sizeof(compressed_pubkey1);
    return_val = rustsecp256k1zkp_v0_7_0_ec_pubkey_serialize(ctx, compressed_pubkey1, &len, &pubkey1, SECP256K1_EC_COMPRESSED);
    assert(return_val);
    ...
    return_val = rustsecp256k1zkp_v0_7_0_ec_pubkey_serialize(ctx, compressed_pubkey2, &len, &pubkey2, SECP256K1_EC_COMPRESSED);
    assert(return_val);
    /* Should be the same size as the size of the output, because we passed a 33 byte array. */
    ...
    return_val = rustsecp256k1zkp_v0_7_0_ecdh(ctx, shared_secret1, &pubkey2, seckey1, NULL, NULL);
    assert(return_val);

    /* Perform ECDH with seckey2 and pubkey1. Should never fail with a verified
     * seckey and valid pubkey */
    return_val = rustsecp256k1zkp_v0_7_0_ecdh(ctx, shared_secret2, &pubkey1, seckey2, NULL, NULL);
    assert(return_val);
