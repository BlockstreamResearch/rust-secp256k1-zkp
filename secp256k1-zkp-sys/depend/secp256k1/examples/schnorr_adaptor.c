...
static int create_keypair(const rustsecp256k1zkp_v0_10_1_context *ctx, rustsecp256k1zkp_v0_10_1_keypair *keypair, rustsecp256k1zkp_v0_10_1_xonly_pubkey *pubkey) {
    ...
    while (1) {
        ...
        if (rustsecp256k1zkp_v0_10_1_keypair_create(ctx, keypair, seckey)) {
            break;
        }
    }
    if(!rustsecp256k1zkp_v0_10_1_keypair_xonly_pub(ctx, pubkey, NULL, keypair)){
		return 0;
	}
	return 1;
}
...
static int create_hop_locks(const rustsecp256k1zkp_v0_10_1_context *ctx, rustsecp256k1zkp_v0_10_1_pubkey *left_lock, rustsecp256k1zkp_v0_10_1_pubkey *right_lock, rustsecp256k1zkp_v0_10_1_pubkey *adaptor_pop, unsigned char *tweak_sum, unsigned char *tweak1, unsigned char *tweak2) {
    while (1) {
        ...
        if (rustsecp256k1zkp_v0_10_1_ec_seckey_verify(ctx, tweak1) && rustsecp256k1zkp_v0_10_1_ec_seckey_verify(ctx, tweak2)) {
            break;
        }
    }
	/* Create left lock = (z + tweak1)*G */
	memcpy(left_lock, adaptor_pop, sizeof(rustsecp256k1zkp_v0_10_1_pubkey));
	if(!rustsecp256k1zkp_v0_10_1_ec_pubkey_tweak_add(ctx, left_lock, tweak1)) {
		return 0;
	}

	/* Create right lock = (z + tweak1 + tweak2)*G */
	memcpy(tweak_sum, tweak1, 32);
	if(!rustsecp256k1zkp_v0_10_1_ec_seckey_tweak_add(ctx, tweak_sum, tweak2)) {
		return 0;
	}
	memcpy(right_lock, adaptor_pop, sizeof(rustsecp256k1zkp_v0_10_1_pubkey));
	if(!rustsecp256k1zkp_v0_10_1_ec_pubkey_tweak_add(ctx, right_lock, tweak_sum)) {
		return 0;
	}
...
int main(void) {
    ...
    rustsecp256k1zkp_v0_10_1_pubkey adaptor_pop;
	rustsecp256k1zkp_v0_10_1_pubkey left_lock;
	rustsecp256k1zkp_v0_10_1_pubkey right_lock;
	rustsecp256k1zkp_v0_10_1_pubkey tmp_pubkey;
    rustsecp256k1zkp_v0_10_1_xonly_pubkey pubkey_a, pubkey_b;
	rustsecp256k1zkp_v0_10_1_keypair keypair_a, keypair_b;
	int ret;

	rustsecp256k1zkp_v0_10_1_context* ctx = rustsecp256k1zkp_v0_10_1_context_create(SECP256K1_CONTEXT_NONE);
	...
	ret = rustsecp256k1zkp_v0_10_1_ec_pubkey_create(ctx, &adaptor_pop, secret_pop);
	assert(ret);
	...
	*/
	...
	ret = rustsecp256k1zkp_v0_10_1_schnorr_adaptor_presign(ctx, presig_ab, tx_ab, &keypair_a, &left_lock, NULL);
	assert(ret);

	/* Bob setup: extracts the left lock from Alice's pre-signature and verifies it */
	ret = rustsecp256k1zkp_v0_10_1_schnorr_adaptor_extract(ctx, &tmp_pubkey, presig_ab, tx_ab, &pubkey_a);
	assert(ret);
	assert(memcmp(&tmp_pubkey, &left_lock, sizeof(left_lock)) == 0);
	/* Bob creates a pre-signature that forwards the payment to Carol */
	ret = rustsecp256k1zkp_v0_10_1_schnorr_adaptor_presign(ctx, presig_bc, tx_bc, &keypair_b, &right_lock, NULL);
	assert(ret);

	/* Carol extracts the right lock from Bob's pre-signature and verifies it */
	ret = rustsecp256k1zkp_v0_10_1_schnorr_adaptor_extract(ctx, &tmp_pubkey, presig_bc, tx_bc, &pubkey_b);
	assert(ret);
	...
	* signature. */
	...
	ret = rustsecp256k1zkp_v0_10_1_ec_seckey_tweak_add(ctx, tmp, tweak_sum);
	assert(ret);
	ret = rustsecp256k1zkp_v0_10_1_schnorr_adaptor_adapt(ctx, sig_bc, presig_bc, tmp);
	assert(ret);
	assert(rustsecp256k1zkp_v0_10_1_schnorrsig_verify(ctx, sig_bc, tx_bc, sizeof(tx_bc), &pubkey_b));

	/* Bob extracts the secret = z + tweak1 + tweak2 from his pre-signature
	* and the BIP340 signature created by Carol. */
	ret = rustsecp256k1zkp_v0_10_1_schnorr_adaptor_extract_sec(ctx, tmp, presig_bc, sig_bc);
	assert(ret);
	...
	* secret = z + tweak1, to produce a valid BIP340 Schnorr signature. */
	ret = rustsecp256k1zkp_v0_10_1_ec_seckey_negate(ctx, tweak2);
	assert(ret);
	ret = rustsecp256k1zkp_v0_10_1_ec_seckey_tweak_add(ctx, tmp, tweak2);
	assert(ret);
	ret = rustsecp256k1zkp_v0_10_1_schnorr_adaptor_adapt(ctx, sig_ab, presig_ab, tmp);
	assert(ret);
	assert(rustsecp256k1zkp_v0_10_1_schnorrsig_verify(ctx, sig_ab, tx_ab, sizeof(tx_ab), &pubkey_a));

	/* Alice extracts the proof of payment = z from her pre-signature
	 * and the BIP340 signature created by Bob. */
	ret = rustsecp256k1zkp_v0_10_1_schnorr_adaptor_extract_sec(ctx, tmp, presig_ab, sig_ab);
	assert(ret);
	ret = rustsecp256k1zkp_v0_10_1_ec_seckey_negate(ctx, tweak1);
	assert(ret);
	ret = rustsecp256k1zkp_v0_10_1_ec_seckey_tweak_add(ctx, tmp, tweak1);
	assert(ret);
