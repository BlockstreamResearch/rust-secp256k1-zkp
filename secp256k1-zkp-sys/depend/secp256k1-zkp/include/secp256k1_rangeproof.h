 ...
 *  comparison, use secp256k1_zkp_pedersen_commitment_serialize and
 *  secp256k1_zkp_pedersen_commitment_parse.
 */
typedef struct {
    unsigned char data[64];
} secp256k1_zkp_pedersen_commitment;

/**
 * Static constant generator 'h' maintained for historical reasons.
 */
SECP256K1_API extern const secp256k1_zkp_generator *secp256k1_zkp_generator_h;
 ...
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_zkp_pedersen_commitment_parse(
 ...
 *  In:     commit:     a pointer to a secp256k1_zkp_pedersen_commitment containing an
 *                      initialized commitment
 */
SECP256K1_API int secp256k1_zkp_pedersen_commitment_serialize(
    const secp256k1_zkp_context* ctx,
...
void secp256k1_zkp_pedersen_context_initialize(secp256k1_zkp_context* ctx);
 ...
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_zkp_pedersen_commit(
  const secp256k1_zkp_context* ctx,
  secp256k1_zkp_pedersen_commitment *commit,
  const unsigned char *blind,
  uint64_t value,
  const secp256k1_zkp_generator *gen
 ...
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_zkp_pedersen_blind_sum(
  const secp256k1_zkp_context* ctx,
 ...
 *
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_zkp_pedersen_verify_tally(
  const secp256k1_zkp_context* ctx,
  const secp256k1_zkp_pedersen_commitment * const* commits,
  size_t pcnt,
  const secp256k1_zkp_pedersen_commitment * const* ncommits,
  size_t ncnt
 ...
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_zkp_pedersen_blind_generator_blind_sum(
  const secp256k1_zkp_context* ctx,
...
/** Initialize a context for usage with Pedersen commitments. */
void secp256k1_zkp_rangeproof_context_initialize(secp256k1_zkp_context* ctx);
 ...
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_zkp_rangeproof_verify(
  const secp256k1_zkp_context* ctx,
  uint64_t *min_value,
  uint64_t *max_value,
  const secp256k1_zkp_pedersen_commitment *commit,
 ...
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_zkp_rangeproof_rewind(
  const secp256k1_zkp_context* ctx,
  unsigned char *blind_out,
  uint64_t *value_out,
 ...
 *
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_zkp_rangeproof_sign(
  const secp256k1_zkp_context* ctx,
  unsigned char *proof,
  size_t *plen,
  uint64_t min_value,
 ...
 *        max_value: pointer to an unsigned int64 which will be updated with the maximum value that commit could have. (cannot be NULL)
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_zkp_rangeproof_info(
  const secp256k1_zkp_context* ctx,
  int *exp,
  int *mantissa,
  uint64_t *min_value,
