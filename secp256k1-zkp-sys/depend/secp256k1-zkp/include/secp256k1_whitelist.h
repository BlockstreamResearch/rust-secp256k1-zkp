 ...
 *
 *  To obtain these properties, instead use secp256k1_zkp_whitelist_signature_parse
 *  and secp256k1_zkp_whitelist_signature_serialize to encode/decode signatures
 ...
 *  of keys for a signature, use `secp256k1_zkp_whitelist_signature_n_keys`.
 */
typedef struct {
    size_t n_keys;
    /* e0, scalars */
    unsigned char data[32 * (1 + SECP256K1_WHITELIST_MAX_N_KEYS)];
} secp256k1_zkp_whitelist_signature;
 ...
 */
SECP256K1_API int secp256k1_zkp_whitelist_signature_parse(
 ...
 */
SECP256K1_API size_t secp256k1_zkp_whitelist_signature_n_keys(
    const secp256k1_zkp_whitelist_signature *sig
 ...
 *
 *  See secp256k1_zkp_whitelist_signature_parse for details about the encoding.
 */
SECP256K1_API int secp256k1_zkp_whitelist_signature_serialize(
    const secp256k1_zkp_context* ctx,
 ...
 */
SECP256K1_API int secp256k1_zkp_whitelist_sign(
  const secp256k1_zkp_context* ctx,
  secp256k1_zkp_whitelist_signature *sig,
  const secp256k1_zkp_pubkey *online_pubkeys,
  const secp256k1_zkp_pubkey *offline_pubkeys,
  const size_t n_keys,
  const secp256k1_zkp_pubkey *sub_pubkey,
 ...
 *         sub_pubkey: the key to be whitelisted
 */
SECP256K1_API int secp256k1_zkp_whitelist_verify(
  const secp256k1_zkp_context* ctx,
  const secp256k1_zkp_whitelist_signature *sig,
  const secp256k1_zkp_pubkey *online_pubkeys,
  const secp256k1_zkp_pubkey *offline_pubkeys,
