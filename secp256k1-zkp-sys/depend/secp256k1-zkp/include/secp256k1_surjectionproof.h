 ...
 *
 *  To obtain these properties, instead use secp256k1_zkp_surjectionproof_parse
 *  and secp256k1_zkp_surjectionproof_serialize to encode/decode proofs into a
 *  well-defined format.
 *
...
typedef struct {
#ifdef VERIFY
    /** Mark whether this proof has gone through `secp256k1_zkp_surjectionproof_initialize` */
    int initialized;
#endif
 ...
 */
SECP256K1_API int secp256k1_zkp_surjectionproof_parse(
 ...
 *
 *  See secp256k1_zkp_surjectionproof_parse for details about the encoding.
 */
SECP256K1_API int secp256k1_zkp_surjectionproof_serialize(
  const secp256k1_zkp_context* ctx,
  unsigned char *output,
  size_t *outputlen,
  const secp256k1_zkp_surjectionproof *proof
 ...
 */
SECP256K1_API size_t secp256k1_zkp_surjectionproof_n_total_inputs(
  const secp256k1_zkp_context* ctx,
  const secp256k1_zkp_surjectionproof* proof
 ...
 *     proof: a pointer to a proof object
 */
SECP256K1_API size_t secp256k1_zkp_surjectionproof_n_used_inputs(
  const secp256k1_zkp_context* ctx,
  const secp256k1_zkp_surjectionproof* proof
) SECP256K1_ARG_NONNULL(1) SECP256K1_ARG_NONNULL(2);
 ...
 *     proof: a pointer to a proof object
 */
SECP256K1_API size_t secp256k1_zkp_surjectionproof_serialized_size(
  const secp256k1_zkp_context* ctx,
  const secp256k1_zkp_surjectionproof* proof
) SECP256K1_ARG_NONNULL(1) SECP256K1_ARG_NONNULL(2);
 ...
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_zkp_surjectionproof_initialize(
  const secp256k1_zkp_context* ctx,
  secp256k1_zkp_surjectionproof* proof,
  size_t *input_index,
  const secp256k1_zkp_fixed_asset_tag* fixed_input_tags,
  const size_t n_input_tags,
  const size_t n_input_tags_to_use,
  const secp256k1_zkp_fixed_asset_tag* fixed_output_tag,
 ...
 * In/Out: proof: The produced surjection proof. Must have already gone through `secp256k1_zkp_surjectionproof_initialize`
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_zkp_surjectionproof_generate(
  const secp256k1_zkp_context* ctx,
  secp256k1_zkp_surjectionproof* proof,
  const secp256k1_zkp_generator* ephemeral_input_tags,
  size_t n_ephemeral_input_tags,
  const secp256k1_zkp_generator* ephemeral_output_tag,
  size_t input_index,
 ...
 *      ephemeral_output_tag: the ephemeral asset tag of the output
 */
SECP256K1_API int secp256k1_zkp_surjectionproof_verify(
  const secp256k1_zkp_context* ctx,
  const secp256k1_zkp_surjectionproof* proof,
  const secp256k1_zkp_generator* ephemeral_input_tags,
  size_t n_ephemeral_input_tags,
