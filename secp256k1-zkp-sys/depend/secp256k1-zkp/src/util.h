...
typedef struct {
    ...
    const void* data;
} secp256k1_zkp_callback;

static SECP256K1_INLINE void secp256k1_zkp_callback_call(const secp256k1_zkp_callback * const cb, const char * const text) {
...
static SECP256K1_INLINE void *checked_malloc(const secp256k1_zkp_callback* cb, size_t size) {
    void *ret = malloc(size);
    if (ret == NULL) {
        secp256k1_zkp_callback_call(cb, "Out of memory");
    }
    return ret;
}

static SECP256K1_INLINE void *checked_realloc(const secp256k1_zkp_callback* cb, void *ptr, size_t size) {
...
SECP256K1_INLINE static int secp256k1_zkp_sign_and_abs64(uint64_t *out, int64_t in) {
...
SECP256K1_INLINE static int secp256k1_zkp_clz64_var(uint64_t x) {
    int ret;
    if (!x) {
        return 64;
    }
