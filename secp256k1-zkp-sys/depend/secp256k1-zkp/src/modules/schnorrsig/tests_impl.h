...
void test_schnorrsig_serialize(void) {
    ...
    memset(in, 0x12, 64);
    CHECK(secp256k1_zkp_schnorrsig_parse(ctx, &sig, in));
    CHECK(secp256k1_zkp_schnorrsig_serialize(ctx, out, &sig));
    CHECK(memcmp(in, out, 64) == 0);
}

void test_schnorrsig_api(secp256k1_zkp_scratch_space *scratch) {
    unsigned char sk1[32];
    unsigned char sk2[32];
    unsigned char sk3[32];
    unsigned char msg[32];
    unsigned char sig64[64];
    secp256k1_zkp_pubkey pk[3];
    secp256k1_zkp_schnorrsig sig;
    const secp256k1_zkp_schnorrsig *sigptr = &sig;
    const unsigned char *msgptr = msg;
    const secp256k1_zkp_pubkey *pkptr = &pk[0];
    ...
    secp256k1_zkp_rand256(sk1);
    secp256k1_zkp_rand256(sk2);
    secp256k1_zkp_rand256(sk3);
    secp256k1_zkp_rand256(msg);
    CHECK(secp256k1_zkp_ec_pubkey_create(ctx, &pk[0], sk1) == 1);
    CHECK(secp256k1_zkp_ec_pubkey_create(ctx, &pk[1], sk2) == 1);
    CHECK(secp256k1_zkp_ec_pubkey_create(ctx, &pk[2], sk3) == 1);

    /** main test body **/
    ecount = 0;
    CHECK(secp256k1_zkp_schnorrsig_sign(none, &sig, &nonce_is_negated, msg, sk1, NULL, NULL) == 0);
    CHECK(ecount == 1);
    CHECK(secp256k1_zkp_schnorrsig_sign(vrfy, &sig, &nonce_is_negated, msg, sk1, NULL, NULL) == 0);
    CHECK(ecount == 2);
    CHECK(secp256k1_zkp_schnorrsig_sign(sign, &sig, &nonce_is_negated, msg, sk1, NULL, NULL) == 1);
    CHECK(ecount == 2);
    CHECK(secp256k1_zkp_schnorrsig_sign(sign, NULL, &nonce_is_negated, msg, sk1, NULL, NULL) == 0);
    CHECK(ecount == 3);
    CHECK(secp256k1_zkp_schnorrsig_sign(sign, &sig, NULL, msg, sk1, NULL, NULL) == 1);
    CHECK(ecount == 3);
    CHECK(secp256k1_zkp_schnorrsig_sign(sign, &sig, &nonce_is_negated, NULL, sk1, NULL, NULL) == 0);
    CHECK(ecount == 4);
    CHECK(secp256k1_zkp_schnorrsig_sign(sign, &sig, &nonce_is_negated, msg, NULL, NULL, NULL) == 0);
    CHECK(ecount == 5);

    ecount = 0;
    CHECK(secp256k1_zkp_schnorrsig_serialize(none, sig64, &sig) == 1);
    CHECK(ecount == 0);
    CHECK(secp256k1_zkp_schnorrsig_serialize(none, NULL, &sig) == 0);
    CHECK(ecount == 1);
    CHECK(secp256k1_zkp_schnorrsig_serialize(none, sig64, NULL) == 0);
    CHECK(ecount == 2);
    CHECK(secp256k1_zkp_schnorrsig_parse(none, &sig, sig64) == 1);
    CHECK(ecount == 2);
    CHECK(secp256k1_zkp_schnorrsig_parse(none, NULL, sig64) == 0);
    CHECK(ecount == 3);
    CHECK(secp256k1_zkp_schnorrsig_parse(none, &sig, NULL) == 0);
    CHECK(ecount == 4);

    ecount = 0;
    CHECK(secp256k1_zkp_schnorrsig_verify(none, &sig, msg, &pk[0]) == 0);
    CHECK(ecount == 1);
    CHECK(secp256k1_zkp_schnorrsig_verify(sign, &sig, msg, &pk[0]) == 0);
    CHECK(ecount == 2);
    CHECK(secp256k1_zkp_schnorrsig_verify(vrfy, &sig, msg, &pk[0]) == 1);
    CHECK(ecount == 2);
    CHECK(secp256k1_zkp_schnorrsig_verify(vrfy, NULL, msg, &pk[0]) == 0);
    CHECK(ecount == 3);
    CHECK(secp256k1_zkp_schnorrsig_verify(vrfy, &sig, NULL, &pk[0]) == 0);
    CHECK(ecount == 4);
    CHECK(secp256k1_zkp_schnorrsig_verify(vrfy, &sig, msg, NULL) == 0);
    CHECK(ecount == 5);

    ecount = 0;
    CHECK(secp256k1_zkp_schnorrsig_verify_batch(none, scratch, &sigptr, &msgptr, &pkptr, 1) == 0);
    CHECK(ecount == 1);
    CHECK(secp256k1_zkp_schnorrsig_verify_batch(sign, scratch, &sigptr, &msgptr, &pkptr, 1) == 0);
    CHECK(ecount == 2);
    CHECK(secp256k1_zkp_schnorrsig_verify_batch(vrfy, scratch, &sigptr, &msgptr, &pkptr, 1) == 1);
    CHECK(ecount == 2);
    CHECK(secp256k1_zkp_schnorrsig_verify_batch(vrfy, scratch, NULL, NULL, NULL, 0) == 1);
    CHECK(ecount == 2);
    CHECK(secp256k1_zkp_schnorrsig_verify_batch(vrfy, scratch, NULL, &msgptr, &pkptr, 1) == 0);
    CHECK(ecount == 3);
    CHECK(secp256k1_zkp_schnorrsig_verify_batch(vrfy, scratch, &sigptr, NULL, &pkptr, 1) == 0);
    CHECK(ecount == 4);
    CHECK(secp256k1_zkp_schnorrsig_verify_batch(vrfy, scratch, &sigptr, &msgptr, NULL, 1) == 0);
    CHECK(ecount == 5);
    CHECK(secp256k1_zkp_schnorrsig_verify_batch(vrfy, scratch, &sigptr, &msgptr, &pkptr, (size_t)1 << (sizeof(size_t)*8-1)) == 0);
    CHECK(ecount == 6);
    CHECK(secp256k1_zkp_schnorrsig_verify_batch(vrfy, scratch, &sigptr, &msgptr, &pkptr, 1 << 31) == 0);
    CHECK(ecount == 7);
...
void test_schnorrsig_bip_vectors_check_signing(const unsigned char *sk, const unsigned char *pk_serialized, const unsigned char *msg, const unsigned char *expected_sig, const int expected_nonce_is_negated) {
    ...
    CHECK(secp256k1_zkp_schnorrsig_sign(ctx, &sig, &nonce_is_negated, msg, sk, NULL, NULL));
    CHECK(nonce_is_negated == expected_nonce_is_negated);
    CHECK(secp256k1_zkp_schnorrsig_serialize(ctx, serialized_sig, &sig));
    CHECK(memcmp(serialized_sig, expected_sig, 64) == 0);

    CHECK(secp256k1_zkp_ec_pubkey_parse(ctx, &pk, pk_serialized, 33));
    CHECK(secp256k1_zkp_schnorrsig_verify(ctx, &sig, msg, &pk));
}

/* Helper function for schnorrsig_bip_vectors
 * Checks that both verify and verify_batch return the same value as expected. */
void test_schnorrsig_bip_vectors_check_verify(secp256k1_zkp_scratch_space *scratch, const unsigned char *pk_serialized, const unsigned char *msg32, const unsigned char *sig_serialized, int expected) {
    const unsigned char *msg_arr[1];
    const secp256k1_zkp_schnorrsig *sig_arr[1];
    const secp256k1_zkp_pubkey *pk_arr[1];
    secp256k1_zkp_pubkey pk;
    secp256k1_zkp_schnorrsig sig;

    CHECK(secp256k1_zkp_ec_pubkey_parse(ctx, &pk, pk_serialized, 33));
    CHECK(secp256k1_zkp_schnorrsig_parse(ctx, &sig, sig_serialized));

    sig_arr[0] = &sig;
    msg_arr[0] = msg32;
    pk_arr[0] = &pk;

    CHECK(expected == secp256k1_zkp_schnorrsig_verify(ctx, &sig, msg32, &pk));
    CHECK(expected == secp256k1_zkp_schnorrsig_verify_batch(ctx, scratch, sig_arr, msg_arr, pk_arr, 1));
}
 ...
 */
void test_schnorrsig_bip_vectors(secp256k1_zkp_scratch_space *scratch) {
    {
        /* Test vector 1 */
    ...
    }
    {
        ...
        secp256k1_zkp_pubkey pk7_parsed;
        /* No need to check the signature of the test vector as parsing the pubkey already fails */
        CHECK(!secp256k1_zkp_ec_pubkey_parse(ctx, &pk7_parsed, pk7, 33));
    }
    {
        /* Test vector 8 */
...
void test_schnorrsig_sign(void) {
    ...
    CHECK(secp256k1_zkp_schnorrsig_sign(ctx, &sig, NULL, msg, sk, NULL, NULL) == 1);

    /* Overflowing secret key */
    memset(sk, 0xFF, sizeof(sk));
    CHECK(secp256k1_zkp_schnorrsig_sign(ctx, &sig, NULL, msg, sk, NULL, NULL) == 0);
    memset(sk, 23, sizeof(sk));

    CHECK(secp256k1_zkp_schnorrsig_sign(ctx, &sig, NULL, msg, sk, nonce_function_failing, NULL) == 0);
    CHECK(secp256k1_zkp_schnorrsig_sign(ctx, &sig, NULL, msg, sk, nonce_function_0, NULL) == 0);
}
...
void test_schnorrsig_sign_verify(secp256k1_zkp_scratch_space *scratch) {
    const unsigned char sk[32] = "shhhhhhhh! this key is a secret.";
    unsigned char msg[N_SIGS][32];
    secp256k1_zkp_schnorrsig sig[N_SIGS];
    size_t i;
    const secp256k1_zkp_schnorrsig *sig_arr[N_SIGS];
    const unsigned char *msg_arr[N_SIGS];
    const secp256k1_zkp_pubkey *pk_arr[N_SIGS];
    secp256k1_zkp_pubkey pk;

    CHECK(secp256k1_zkp_ec_pubkey_create(ctx, &pk, sk));

    CHECK(secp256k1_zkp_schnorrsig_verify_batch(ctx, scratch, NULL, NULL, NULL, 0));

    for (i = 0; i < N_SIGS; i++) {
        secp256k1_zkp_rand256(msg[i]);
        CHECK(secp256k1_zkp_schnorrsig_sign(ctx, &sig[i], NULL, msg[i], sk, NULL, NULL));
        CHECK(secp256k1_zkp_schnorrsig_verify(ctx, &sig[i], msg[i], &pk));
        sig_arr[i] = &sig[i];
        msg_arr[i] = msg[i];
        pk_arr[i] = &pk;
    }

    CHECK(secp256k1_zkp_schnorrsig_verify_batch(ctx, scratch, sig_arr, msg_arr, pk_arr, 1));
    CHECK(secp256k1_zkp_schnorrsig_verify_batch(ctx, scratch, sig_arr, msg_arr, pk_arr, 2));
    CHECK(secp256k1_zkp_schnorrsig_verify_batch(ctx, scratch, sig_arr, msg_arr, pk_arr, 4));
    CHECK(secp256k1_zkp_schnorrsig_verify_batch(ctx, scratch, sig_arr, msg_arr, pk_arr, N_SIGS));

    {
        /* Flip a few bits in the signature and in the message and check that
         * verify and verify_batch fail */
        size_t sig_idx = secp256k1_zkp_rand_int(4);
        size_t byte_idx = secp256k1_zkp_rand_int(32);
        unsigned char xorbyte = secp256k1_zkp_rand_int(254)+1;
        sig[sig_idx].data[byte_idx] ^= xorbyte;
        CHECK(!secp256k1_zkp_schnorrsig_verify(ctx, &sig[sig_idx], msg[sig_idx], &pk));
        CHECK(!secp256k1_zkp_schnorrsig_verify_batch(ctx, scratch, sig_arr, msg_arr, pk_arr, 4));
        sig[sig_idx].data[byte_idx] ^= xorbyte;

        byte_idx = secp256k1_zkp_rand_int(32);
